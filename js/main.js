...
const ui = {
  ...
  progressExpirationOutlivedView: document.getElementById(
    "progressElapsedExpiration"
  ),
...
async function getExpectancyStats() {
  const res = await fetch("../data/life-expectancy.json");
...
function updateCurrentProfileInStorage(keyName, value) {
  ...
  localStorage.setItem(
    data.currentProfile,
    JSON.stringify(retrievedProfileData)
  );
...
function getLifeExpectancyAndExpiration() {
  data.expiration.setFullYear(
    new Date(data.answers.dob).getFullYear() +
      data.countryList[data.answers.country][data.answers.gender]
  );
...
function saveDayEntryToLocalStorage(dayId, attribute, content) {
  ...
  if (currentStorageEntries[dayId] === undefined) {
    currentStorageEntries[dayId] = {
      message: "",
      image: "/images/imagePlaceholder.png",
    };
  }
  currentStorageEntries[dayId][attribute] = content;
  updateCurrentProfileInStorage("entries", currentStorageEntries);
...
function showStatsProgressNumbers() {
  ...
  ui.statsLifespan.innerText = `${new Date(data.answers.dob).getDate()}.${
    new Date(data.answers.dob).getMonth() + 1
  }.${new Date(
    data.answers.dob
  ).getFullYear()} - ${data.expiration.getDate()}.${
    data.expiration.getMonth() + 1
  }.${data.expiration.getFullYear()}`;
  ...
  let currentAge =
    data.today.getFullYear() - new Date(data.answers.dob).getFullYear();

  //--Still within the expectancy
  if (currentAge <= data.expectancy) {
    ui.progressExtra.style.display = "none";
    ui.progressElapsed.style.width = `${
      (currentAge / Math.round(data.expectancy)) * 100
    }%`;
    ui.progressRemaining.style.width = `${
      100 - (currentAge / Math.round(data.expectancy)) * 100
    }%`;
  }
  //--Already outlived the expectancy
    ...
    ui.progressElapsed.style.width = `${
      100 - (aboveExpectancy / data.expectancy) * 100
    }%`;
    ui.progressExtra.style.display = "block";
    ui.progressExtra.style.width = `${
      (aboveExpectancy / data.expectancy) * 100
    }%`;
...
function visualizeWithinExpectactionDays() {
  ...
  if (new Date(data.answers.dob).getDate() !== 1) {
    ...
    document
      .getElementById("year-body" + days[0].getFullYear())
  ...
  days.forEach((day) => {
    ...
    if (day.getDate() === 1) {
      ...
      monthLabel.classList.add("monthLabel");
      document
        .getElementById("year-body" + day.getFullYear())
        .appendChild(monthLabel);
    }
...
function highlightOutlivedExpectancy() {
  if (data.outlivedExpectancy === true) {
    let lastExpectedDay = `${new Date(data.expiration).getFullYear()}-${
      new Date(data.expiration).getMonth() + 1
    }-${new Date(data.expiration).getDate()}`;
    ...
    if (data.setLastExpectedDayDefaultMessage === undefined) {
      saveDayEntryToLocalStorage(
        ui.lastExpectedDayId,
        "message",
        "Based on the average life expectancy, this was the last day of your life! Congratulations if you can see this message! All following days will be highlight with a special color so that you can remind yourself of enjoying them even more!"
      );
      updateCurrentProfileInStorage("setLastExpectedDayDefaultMessage", "true");
    }
...
function highlightFirstBirthday() {
  let firstBirthday = `${new Date(data.answers.dob).getFullYear()}-${
    new Date(data.answers.dob).getMonth() + 1
  }-${new Date(data.answers.dob).getDate()}`;
  ...
  if (data.setFirstBirthdayDefaultMessage === undefined) {
    ...
    saveDayEntryToLocalStorage(
      ui.firstBirthdayId,
      "message",
      "This is the day when you were born!"
    );
...
function highlightToday() {
  let todayDate = new Date();
  let todaySelector = `${todayDate.getFullYear()}-${
...
function displayModal(input) {
  ...
  if (getDayEntryFromLocalStorage(input.clickedDayId).message === undefined) {
    ...
    ui.dayEntryForm.value = getDayEntryFromLocalStorage(
      input.clickedDayId
    ).message;
  }

  if (getDayEntryFromLocalStorage(input.clickedDayId).image !== undefined) {
    ui.dayEntryImage.src = getDayEntryFromLocalStorage(
      input.clickedDayId
    ).image;
  } else {
...
ui.controllersYearSelect.addEventListener("change", (e) => {
  e.preventDefault();
  let domPositionOfSelectedYear = document.getElementById(
    "dom-year" + ui.controllersYearSelect.value
  ).offsetTop;
...
function attachActionToDays() {
  Array.from(document.querySelectorAll(".day")).forEach((item) => {
    item.addEventListener("click", () => {
      let clickedDayId = `${item.getAttribute("data-year")}-${item.getAttribute(
        "data-month"
...
ui.dayEntryImage.addEventListener("click", (e) => {
  e.preventDefault();
  if (
    ui.imageUrlInsertForm.style.display === "" ||
    ...
    if (getDayEntryFromLocalStorage(data.openedModalId).image === undefined) {
      ...
      ui.imageUrlInsertField.value = getDayEntryFromLocalStorage(
        data.openedModalId
      ).image;
    }
  } else {
